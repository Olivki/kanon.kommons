 ...
 *
 * As this is just a facade/wrapper, nothing actually gets stored in it. This means that [entries], [keys],
 * [values] and [size] are just references that are being transformed into the appropriate collections, this means that
 * explicitly calling any of them will come with some overhead, and thus it's not recommended to explicitly call them.
 *
 ...
 */
class AttributeMap @PublishedApi internal constructor(private val parent: Element) : MutableMap<String, String> {
     ...
     * @since DOM Level 2
     */
    override fun containsKey(key: String): Boolean = parent.hasAttribute(key)

    /**
     * Returns `true` when an attribute with the given [value] is specified on this element, `false` otherwise.
     *
     * @param value The value to look for.
     *
     * @return `true` when an attribute with the given [value] is specified on this element, `false` otherwise.
     *
     * @since DOM Level 2
     */
     ...
     */
    override fun get(key: String): String? = parent.getAttribute(key)

    override fun isEmpty(): Boolean = nodes.isEmpty()

    override fun clear() {
        throw NotImplementedError("Clearing operations on AttributeMap is not supported.")
    }
     ...
     *
     * @param key The name of the attribute to create or alter.
     ...
     * @since DOM Level 2
     */
    operator fun set(namespaceURI: String, qualifiedName: String?, value: String) {
     ...
     * @since DOM Level 2
     */
    operator fun minusAssign(nsAttr: Pair<String?, String>) {
        parent.removeAttributeNS(nsAttr.first, nsAttr.second)
    }
     ...
     * @since DOM Level 2
     */
    operator fun get(namespaceURI: String?, localName: String): String =
        parent.getAttributeNS(namespaceURI, localName)!!
     ...
     * @since DOM Level 2
     */
    operator fun contains(attrPair: Pair<String?, String>): Boolean =
        parent.hasAttributeNS(attrPair.first, attrPair.second)
     ...
     * @since DOM Level 3
     */
    operator fun set(name: String, isId: Boolean) {
     ...
     * @since DOM Level 3
     */
    operator fun set(namespaceURI: String, localName: String, isId: Boolean) {
        parent.setIdAttributeNS(namespaceURI, localName, isId)
    }
}

/**
 * Refer to [AttributeMap] documentation for information.
 *
 * @see AttributeMap
 */
val Element.attributeMap: AttributeMap
 ...
 * @since 0.4.0
 */
inline fun Element.appendElement(tagName: String): Element =
 ...
 * @since 0.4.0
 */
inline fun Element.appendElementNS(namespaceURI: String?, qualifiedName: String): Element =
 ...
 * @since 0.4.0
 */
inline fun Element.appendComment(data: String): Comment =
 ...
 * @since 0.4.0
 */
inline fun Element.appendTextNode(data: String): Text =
 ...
 * @since 0.4.1
 */
inline fun Element.appendTextContainer(name: String, data: String): Element {
 ...
 * @since 0.4.0
 */
inline fun Element.appendCDataSection(data: String): CDATASection =
 ...
 * @since 0.4.0
 */
inline fun Element.appendProcessingInstruction(target: String, data: String): ProcessingInstruction =
 ...
 * @since 0.4.0
 */
inline fun Element.appendEntityReference(name: String): EntityReference =
