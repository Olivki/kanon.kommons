...
import moe.kanon.kommons.collections.iterables.mapArray
import moe.kanon.kommons.func.tuples.associate
...
class TypeResolver {
    ...
    private fun resolveTypes(types: Array<Type>): Array<Type> = types.asIterable().mapArray { resolveType(it) }
    ...
    private open class TypeTable {
        ...
        /*internal fun where(mappings: Map<TypeVariableKey, Type>): TypeTable = TypeTable(map.toMutableMap().apply {
            putAll(
                mappings.throwOnMatch(
                    { (v, t) -> !v.equalsType(t) },
                    { (v) -> IllegalArgumentException("Type variable <$v> is bound to itself") }
                )
            )
        })*/
...
@DslMarker
annotation class TypeOf

@TypeOf
inline fun <reified Type> typeOf() = object : TypeToken<Type>() {}
 ...
 * `ownerType`.
 */
fun Type?.newParameterizedTypeWithOwner(
    rawType: Class<*>,
...
fun Class<*>.newParameterizedType(arguments: Array<Type>): ParameterizedType =
    ParameterizedTypeImpl(
        ClassOwnership.JVM_BEHAVIOR.getOwnerType(this),
        this, arguments
    )
...
private enum class ClassOwnership {
    ...
    companion object {
        ...
        private fun detectJvmBehavior(): ClassOwnership {
            ...
            for (behavior in values()) if (behavior.getOwnerType(LocalClass::class.java) == parameterizedType.ownerType) return behavior
 ...
 * `bounds`.
 */
fun <D : GenericDeclaration> newArtificialTypeVariable(
    declaration: D,
    name: String,
    bounds: Array<Type>
): TypeVariable<D> =
...
fun Type.subtypeOf(): WildcardType = WildcardTypeImpl(createArray(0), arrayOf(this))

/** Returns a new [WildcardType] with `lowerBound`.  */
fun Type.supertypeOf(): WildcardType = WildcardTypeImpl(arrayOf(this), arrayOf(Any::class.java))
...
private class TypeVariableInvocationHandler(internal val typeVariableImpl: TypeVariableImpl<*>) : InvocationHandler {
    ...
    companion object {
        private val typeVariableMethods: Map<String, Method> = TypeVariableImpl::class.java.methods
            .filter { it.declaringClass == TypeVariableImpl::class.java }
            .associate {
                try {
